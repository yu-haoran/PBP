...
df_train = pd.read_csv('bargain_dataset_train_seed1.csv')
df_val = pd.read_csv('bargain_dataset_val_seed1.csv')
df_test = pd.read_csv('bargain_dataset_test_seed1.csv')
...
yc_train = df_train.iloc[:, -2].values
yd_train = df_train.iloc[:, -1].values

x_val = df_val.iloc[:, :-2].values
yc_val = df_val.iloc[:, -2].values
yd_val = df_val.iloc[:, -1].values

x_test = df_test.iloc[:, :-2].values
yc_test = df_test.iloc[:, -2].values
yd_test = df_test.iloc[:, -1].values

# convert yc into a value between 0 and 1, based on the latest two asking prices
for int in range(len(x_train[:, 0])):
    if yd_train[int] == 3:
        brecent, srecent = findrecentask(x_train[int])
        if brecent != srecent:
            yc_train_ratio = yc_train[int] / x_train[int, 2]  # x_train[int, 2] is s0
        else:
            yc_train_ratio = -100
        yc_train[int] = yc_train_ratio
for int in range(len(x_val[:, 0])):
    if yd_val[int] == 3:
        brecent, srecent = findrecentask(x_val[int])
        if brecent != srecent:
            yc_val_ratio = yc_val[int] / x_val[int, 2]  # x_val[int, 2] is s0
        else:
            yc_val_ratio = -100
        yc_val[int] = yc_val_ratio
for int in range(len(x_test[:, 0])):
    if yd_test[int] == 3:
        brecent, srecent = findrecentask(x_test[int])
        if brecent != srecent:
            yc_test_ratio = yc_test[int] / x_test[int, 2]  # x_test[int, 2] is s0
        else:
            yc_test_ratio = -100
        yc_test[int] = yc_test_ratio

le = preprocessing.LabelEncoder()
le.fit([1, 2, 3])
yd_train = le.transform(yd_train)
yd_val = le.transform(yd_val)
yd_test = le.transform(yd_test)

x_train, yd_train, yc_train = shuffle(x_train, yd_train, yc_train)
x_val, yd_val, yc_val = shuffle(x_val, yd_val, yc_val)
x_test, yd_test, yc_test = shuffle(x_test, yd_test, yc_test)
...
for iterationind in range(numberofiterations):
    ...
    x_train_withl_expand, yd_train_expand, yc_train_expand, weight_of_data = augmentxtrain_weighted(x_train.copy(), yd_train.copy(), yc_train.copy(), Lset, sellerlest_norm.copy())
    ...
    threshold_sampleweight = 0.1 * 1/len(pi)
    ...
    cp_callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=60, restore_best_weights=True)
